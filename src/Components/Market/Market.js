...
import { Divider, List, ListItem, ListItemText, Box, Drawer, Typography, Grid, Avatar, Container } from '@mui/material';
...
const Market = () => {
    ...
    const investmentData = {
        "balances": ["btc", "eth", "matic", "aave"]
    };
    const circle = {
        "circles": ["btc", "eth", "aave", "matic", "link"]
    };
    ...
    const [investData] = useState(investmentData.balances);
    const [circleData] = useState(circle.circles)
    ...
    const handleSearch = (e) => {
        ...
        const filteredRows = coins.filter((row) => {
            return row.symbol.toLowerCase().includes(e.toLowerCase());
        });
            ...
            <Box sx={{ display: 'flex' }} style={{ backgroundColor: '#2a2e30' }}>
                ...
                >
                    ...
                    <List style={{ overflow: 'auto' }} sx={{
                        ...
                        {searchedData.length === 0 ? coins.map(({ symbol }, index) => (
                            ...
                            <div style={{ backgroundColor: '#2a2e30' }}>
                                {selectedCoin ?
                                    <Grid container justify="space-between" style={{ backgroundColor: '#2a2e30' }}>
                                        ...
                                        <Typography variant='h5' inline align='right' color="common.white" style={{ padding: "25px", marginLeft: "20px" }}>$ {(coins.find(x => x.symbol === selectedCoin).current_price)}</Typography>
                                        ...
                                        {circleData.includes(selectedCoin) ?
                                            <Link to={'/btccircle'} style={{ textDecoration: "none" }}>
                                                <IconButton sx={{ marginLeft: "100px", marginTop: "15px" }} style={{ color: 'white' }} onClick={(e) => { }}>
                                                    <PeopleIcon style={{ fontSize: 35 }} />Circle
                                                </IconButton></Link>
                                            :
                                            <div></div>
                                        }
                                        {investData.includes(selectedCoin) ?
                                            < Link to={'/investmentportfolio'} style={{ textDecoration: "none" }}>
                                                <IconButton sx={{ marginLeft: "200px", marginTop: "15px" }} style={{ color: 'white' }} onClick={(e) => { }}>
                                                    <AccountBalanceWalletIcon style={{ fontSize: 35 }} />Wallet
                                                </IconButton></Link>
                                            :
                                            <div></div>
                                        }
